namespace Quantum {
  ...
  unsafe partial struct QuantumDemoInputTopDown {
    public const int INPUT_MAX_MAGNITUDE = 10;
    ...
    public static implicit operator Input(QuantumDemoInputTopDown tInput) {
      ...
      if (direction != default) {
        ...
        // Clamp and normalize so the mult below doesn't overflow
        if (magnitude > INPUT_MAX_MAGNITUDE) magnitude = INPUT_MAX_MAGNITUDE;
        magnitude /= INPUT_MAX_MAGNITUDE;
    ...
    public static implicit operator QuantumDemoInputTopDown(Input input) {
      ...
      if (encodedAngle != default) {
        ...
        // Multiply back to the maximum allowed range
        var magnitude = (((FP)encodedMagnitude) / 255) * INPUT_MAX_MAGNITUDE;
